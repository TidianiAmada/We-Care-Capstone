...
import com.saraya.app.entity.ErrorMessage;
import com.saraya.app.util.WeCareConstants;
...
public class ExceptionControllerAdvice {
	...
	//Handler for exceptions other than NoSuchUserException and validation exceptions
 	@ExceptionHandler(Exception.class)
	public ResponseEntity<ErrorMessage> exceptionHandler2(Exception ex) 
 	{
		 ErrorMessage error = new ErrorMessage();
	     error.setErrorCode(HttpStatus.INTERNAL_SERVER_ERROR.value());
	     error.setMessage(environment.getProperty(WeCareConstants.GENERAL_EXCEPTION_MESSAGE.toString()));
	     return new ResponseEntity<>(error, HttpStatus.OK);
	}
	
	//Handler for NoSuchUserException
